...
function widendam_api_get_iframe_url()
{
    return widendam_api_call("iframe");
}
...
function widendam_api_call($method)
{
    ...
    if (isset($accessToken) && isset($endpoint))
    {
        ...
        $options = array('headers' => $headers, 'timeout' => 60);
...
function widendam_get_endpoint($method)
{
    $collectiveDomain =  get_option('widen_domain');
    if ($method == "iframe") {
        return "https://api.widencollective.com/v2/integrations/url/";
    }
