...
@section('content')
<?php
$mh_text_color = get_field('prod_text_color', 'option');
$mh_backgroundImg = get_field('prod_background_img', 'option');
?>
<style>
  .marq-h-content h1 {
...
<section id="marqHalf">
  <div class="row align-justify align-middle half-half">
    <div class="columns small-12 medium-12 large-5 marq-h-content">
      @if (get_field('prod_headline', 'option'))
      <h1>{{ the_field('prod_headline', 'option') }}</h1>
      @endif
      @if (get_field('prod_subheadline', 'option'))
      <p class="head2-sub">{{ the_field('prod_subheadline', 'option') }}</p>
      @endif
      @if (get_field('prod_marquee_cta', 'option'))
      @php $link = get_field('prod_marquee_cta', 'option'); @endphp
...
<section class="components bp">
  ...
  if ( have_rows('prod_all_components', 'option') ) {
    while ( have_rows('prod_all_components', 'option') ) {
      the_row();
