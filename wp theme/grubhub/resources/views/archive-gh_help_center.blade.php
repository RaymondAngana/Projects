...
@section('content')
<?php
$mh_text_color = get_field('help_text_color', 'option');
$mh_backgroundImg = get_field('help_background_img', 'option');
?>
<style>
  .marq-h-content h1 {
...
<section id="marqHalf">
  <div class="row align-justify align-middle half-half">
    <div class="columns small-12 medium-12 large-5 marq-h-content">
      @if (get_field('help_headline', 'option'))
      <h1>{{ the_field('help_headline', 'option') }}</h1>
      @endif
      @if (get_field('help_subheadline', 'option'))
      <p class="head2-sub">{{ the_field('help_subheadline', 'option') }}</p>
      @endif
      @if (get_field('help_marquee_cta', 'option'))
      @php $link = get_field('help_marquee_cta', 'option'); @endphp
    ...
    <div class="card-col columns large-4 medium-6 small-12" data-equalizer-watch>
      <a href="{{ the_permalink() }}" class="card-link">
        <div class="card img-card">
          ...
          <h4 class="blueberry even">{!! wp_trim_words(get_the_title(), 10, '...') !!}</h4>
...
<section class="components bp">
  ...
  if ( have_rows('help_all_components', 'option') ) {
    while ( have_rows('help_all_components', 'option') ) {
      the_row();
