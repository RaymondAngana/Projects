...
@section('content')
<?php
$mh_text_color = get_field('res_text_color', 'option');
$mh_backgroundImg = get_field('res_background_img', 'option');
?>
<style>
  .marq-h-content h1 {
...
<section id="marqHalf">
  <div class="row align-justify align-middle half-half">
    <div class="columns small-12 medium-12 large-5 marq-h-content">
      @if (get_field('res_headline', 'option'))
      <h1>{{ the_field('res_headline', 'option') }}</h1>
      @endif
      @if (get_field('res_subheadline', 'option'))
      <p class="head2-sub">{{ the_field('res_subheadline', 'option') }}</p>
      @endif
      <div class="subscription-form">
        <h3 class="text-left medium head3-special">{{ the_field('res_form_headline', 'option') }}</h3>
        <div class="form-wrap"> 
          @if (get_field('res_formID', 'option'))
          @php
          $mh_formID = get_field('res_formID', 'option');
          inc_form($mh_formID);
          @endphp
          @else
...
<section class="components bp">
  ...
  if ( have_rows('res_all_components', 'option') ) {
    while ( have_rows('res_all_components', 'option') ) {
      the_row();
