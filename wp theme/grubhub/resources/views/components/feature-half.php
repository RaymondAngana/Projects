...
if ($components) :
  ...
  if (!empty($heading['text'])) {
   $style = ($location == 'top') ? 'h2' : 'h3';
   $header = print_header($heading, $style);
...
?>
<section class="feature-half" style="<?php echo $padding['padding_top'] != '' ? "padding-top: " . $padding['padding_top'] . 'px; ' : ''; echo $padding['padding_bottom'] != '' ? "padding-bottom: " . $padding['padding_bottom']. "px" : ''; ?>">
  <?php
  $html = '';
  $html .= ($location == 'top') ? "<div class='row heading-top'>$header</div>" : '';
  $html .= '<div class="row align-center ' . (($location == 'top') ? 'content-top ' : 'align-middle ') . $wrapper . ' ' . $position . ' ' . $location . '">';
