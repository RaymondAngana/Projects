...
<section id="marqHalf">
  <div class="row align-justify align-middle half-half">
    <div class="columns small-12 medium-5 large-5 marq-h-content">
      @if (get_field('headline_type', $postID) === 'text')
      <h1>{{ the_field('mh_headline', $postID) }}</h1>
      @else
      @php $headlineImage = get_field('mh_image_headline', $postID) @endphp
      <h1 class="img-headline"><img src="{{ $headlineImage['url'] }}" alt="{{ $headlineImage['alt'] }}"></h1>
      @endif
...
<section class="blog-wrap s-grid-cards gh-cards">
  <div class="row">
    <div class="columns large-3 medium-12 small-12 rail-wrap">
      <div class="left-rail top">
        ...
        <ul class="show-for-large">
          ...
          <li class="{{ $cat->term_id === $cat_id ? 'active' : '' }}"><a href="{{ get_category_link($cat->term_id) }}">{{ $cat->name }}</a></li>
          @endforeach
        </ul>
