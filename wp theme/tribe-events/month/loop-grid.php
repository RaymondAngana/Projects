	...
	<table class="tribe-events-calendar">
		...
		<tr>
			<?php foreach ( $days_of_week as $day ) : ?>
				<th scope="col" id="tribe-events-<?php echo esc_attr( strtolower( $day ) ); ?>" title="<?php echo esc_attr( $day ); ?>" data-day-abbr="<?php echo esc_attr( $wp_locale->get_weekday_abbrev( $day ) ); ?>"><?php echo $day ?></th>
			<?php endforeach; ?>
		</tr>
		</thead>
		<tbody>
		<tr>
			<?php $row = 0; while ( tribe_events_have_month_days() ) : tribe_events_the_month_day(); ?>
		...
		<tr>
			...
			// Add scope if first TD of the row.
			$add_scope = ((++$row%7) == 1) ? 'scope="row"' : '';
			?>

			<td <?=$add_scope;?> class="<?php tribe_events_the_month_day_classes() ?>"
